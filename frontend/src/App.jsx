...
function Btn({children,onClick,disabled,variant,busy,help,style}){const a=variant==='danger'?C.red:variant==='warn'?C.amber:variant==='green'?C.green:C.cyan;return<button onClick={onClick} disabled={disabled||busy} title={help||''} style={{padding:'7px 15px',fontSize:11,...mono,background:`${a}15`,color:disabled?C.muted:a,border:`1px solid ${disabled?C.dim:a}44`,borderRadius:6,cursor:disabled?'not-allowed':'pointer',opacity:busy?0.6:1,...(style||{})}}>{busy?'⏳...':children}</button>;}
      ...
      {tab==='pipeline'&&<div>
        ...
        <Card title="Aktionen">
          ...
          <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
            ...
            <Btn onClick={()=>act('export',async()=>{const r=await apiFetch('/api/export/performance');const blob=await r.blob();const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`bot-performance-${new Date().toISOString().slice(0,10)}.json`;a.click();URL.revokeObjectURL(url);setMsg('✅ Performance-Daten exportiert — JSON-Datei gespeichert');})} busy={busy.export} help="Exportiert alle Trades, Predictions, Metriken als JSON zum Hochladen für Analyse">📊 Performance Export</Btn>
